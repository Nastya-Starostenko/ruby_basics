...
class Main
  ...
  def set_values
    ['Odessa','Kharkiv', 'Kiev', 'Moscow', 'Lviv'].each do |city|
  ...
  def create_train(type)
    ...
    begin
      ...
      trains << type == :cargo ? CargoTrain.new(wagons, train_number(number)) : PassengerTrain.new(wagons, train_number(number))
  ...
  def selected_train
    puts "Select train"
    trains.each_with_index { |train, index| puts "#{index + 1} - train #{train.number}, #{train.type}, route: #{train.route.info}" }
    index = gets.to_i - 1
    trains[index]
  end
