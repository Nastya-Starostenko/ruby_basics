...
class Train
  ...
  def initialize(type, wagons, number, route)
    ...
    add_wagons(wagons)
    validate!
    add_route(route) unless route.nil?
    @@trains << self
    register_instance
  end
  ...
  def add_wagons(wagons)
    ...
    wagons.keep_if { |wagon| wagon.type == type && wagon.train.nil? }.each do |wagon|
      self.wagons << wagon
      wagon.attach_train(self)
    end
  end
  ...
  def validate!
    ...
    errors.unshift(self.class.name) unless errors.empty?
